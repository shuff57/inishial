...
import { freshEnv, seedStudent, seedSchoolRoster, seedAccount, seedSyllabus, jsonRequest, cookieFrom, mailEnv, MAIL_FROM, MAIL_REPLY_TO } from './helpers.mjs';
...
test('the mail carries both usernames, and never the word null', async () => {
  // Sign-in is username + code, so a mail with only the codes is two half
  // credentials. It also used to interpolate an absent parent_username
  // straight into the body: every identity created before migration 0012 was
  ...
  try {
    ...
    // A legacy row: registered before parent_username existed.
    env._raw.prepare('UPDATE student_identities SET parent_username = NULL').run();
    await post(env, { student_ext_id: '904511', last: 'Alvarez', email: 'family@example.com' });
    // `body` is both MIME parts joined (see the capture above), so this covers
    // the HTML alternative as well as the plain-text one.
    assert.ok(!/null/i.test(sent[0].body), `"null" reached a parent's inbox: ${sent[0].body}`);
    // The student's half still goes, so the line that CAN be sent is not
    // suppressed along with the one that cannot.
    assert.match(sent[0].body, /904511@s1/, "the student's username is a recovery path");

    // And the normal row carries both.
    sent.length = 0;
    env._raw.prepare("UPDATE student_identities SET parent_username = '904511@p1'").run();
    await post(env, { student_ext_id: '904511', last: 'Alvarez', email: 'family@example.com' });
    assert.match(sent[0].body, /904511@p1/, "the parent's username is the only copy they get");
    assert.match(sent[0].body, /904511@s1/);
  } finally { restore(); }
});
...
test('the code that was mailed opens a parent session at sign-in', async () => {
  ...
  try {
    ...
    // Username out of the same mail. Sign-in takes both, so the email is only
    // a working recovery path if it carries both -- it used to print the
    // username as the literal word "null".
    const username = sent[0].body.match(/username:\s+(\S+)/)[1];
    const loginRes = await login({
      request: jsonRequest('https://x/api/sign/login', { username, code }),
      env,
    });
...
test('a wrong student ID gives the same no-oracle message as login', async () => {
  ...
  try {
    const res = await post(env, { student_ext_id: '0000000', last: 'Alvarez', email: 'x@example.com' });
...
test('a parent-supplied email that differs from the roster is rejected once the parent email is set', async () => {
  ...
  try {
    ...
    assert.equal(res.status, 400);
    assert.equal(sent.length, 0, 'no mail sent when the override is rejected');
  } finally { restore(); }
});
...
test('ten teachers of forty on one IP in one window are not limited', async () => {
  ...
  try {
    ...
    const codes = [];
    for (let i = 0; i < 120; i++) {
      const res = await post(env, { student_ext_id: `9060${String(i).padStart(3, '0')}`, last: `Fam${i}`, email: `fam${i}@example.com` });
...
test('a different email than the one on file is refused outright', async () => {
  const { env, sent, restore } = mailEnv();
  try {
    for (let i = 0; i < 3; i++) {
    ...
    for (let i = 0; i < 3; i++) {
      const res = await post(env, { student_ext_id: `90700${String(i).padStart(2, '0')}`, last: `Fam${i}`, email: `attacker+${i}@example.com` });
      assert.equal(res.status, 400, 'a different email than the one on file is refused');
    }
    assert.equal(sent.length, 0, 'no mail was sent for any of the mismatched requests');
  } finally { restore(); }
});
...
test('a stranger requesting a shared student ID from the wrong school never reaches the other family', async () => {
  ...
  try {
    ...
    const res = await post(env, {
      student_ext_id: '123456', last: 'Whitaker', email: 'whitaker.family@example.com', school_id: whitaker.schoolId,
    });
    assert.equal(res.status, 200);
...
test('the same shared ID from the OTHER school reaches the other family, symmetrically', async () => {
  ...
  try {
    ...
    const res = await post(env, {
      student_ext_id: '123456', last: 'Reyes', email: 'reyes.family@example.com', school_id: reyes.schoolId,
    });
    assert.equal(res.status, 200);
    assert.equal(sent.length, 1);
